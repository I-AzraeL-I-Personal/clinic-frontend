...
export default {
  ...
  methods: {
    async sendRequest() {
      try {
        if (this.type === 'create') {
          ...
          await axios.post(`/doctor`, this.register, config)
          this.$forceUpdate()
        } else if (this.type === 'update') {
          const userResponse = await axios.put('/auth/users/' + this.$store.getters.uuid, this.registerUser)
          const doctorResponse = await axios.put('/doctor/' + this.$store.getters.uuid, this.register)
          this.registerUser = userResponse.data
          this.register = doctorResponse.data
        }
      } catch(error) {
    ...
    async fetchUserData() {
      try {
        const response = await axios.get(`/${this.$store.getters.role}/${this.$store.getters.uuid}`, { params: { contact: true } })
        this.registerUser = { 
